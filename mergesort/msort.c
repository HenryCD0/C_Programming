...
void msort(Student * base, int nel, int (*compar)(const void *, const void *)) {
	...
	//FILL IN
	int nel2 = nel - midpoint;
	msort(base + midpoint, nel2, compar);
	
	//3a. Merge the two arrays (use merge)

	//FILL IN
	Student *merged = merge(base, nel1, base + midpoint, nel2, compar);
...
Student * merge(Student * base1, int nel1, Student * base2, int nel2, int (*compar)(const void *, const void *)) {
	...
	//FILL IN
	Student *output = malloc((nel1 + nel2) * sizeof(Student));
	if(output == NULL) {
		return false;
	}
