...
// program.
  ...
  void helper(CardDeck leftDeck, CardDeck rightDeck, CardDeck *result, int left, int right, int out) {
    ...
    if(right == rightDeck.size) {
      ...
      result->cards[out] = leftDeck.cards[left];
      helper(leftDeck, rightDeck, result, (left+ 1), right, (out + 1));

      result->cards[out] = rightDeck.cards[right];
      helper(leftDeck, rightDeck, result, left, (right + 1), (out + 1));
    }
  }
...
helper(leftDeck, rightDeck, &result, left, right, out);
